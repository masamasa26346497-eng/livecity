// tools/experiments/mission35x_browser_qa.mjs
// [Mission 35X §14/§15] 都市ディテールを足した結果を実機で確かめる。
//   MISSION35X_PHASE=before|after で before/after を撮り分け、同じ地点・同じカメラで比較する。
...
const URL_ = process.env.MISSION35X_URL || 'http://localhost:8080/osaka_3d_buildings.ward-ux-v1.html';
const PHASE = process.env.MISSION35X_PHASE || 'after';
const OUT_DIR = 'data/reports/mission35x-urban-detail-layer';
...
export const SPOTS = [
  ...
  { id: 'nakanoshima', name: '中之島(河川沿い)', lat: 34.6937, lon: 135.4956, r: 620, phDeg: 26 },
  { id: 'shinsaibashi', name: '心斎橋', lat: 34.6723, lon: 135.5010, r: 520, phDeg: 38 },
  { id: 'park-detail', name: '大阪城公園', lat: 34.6873, lon: 135.5262, r: 700, phDeg: 34 },
];
...
const ROAD_STATE = `(() => {
  ...
  const ud = (typeof window.__URBAN_DETAIL_DEBUG__ === 'function') ? window.__URBAN_DETAIL_DEBUG__() : null;
  return JSON.stringify({ groups: byName, colors: [...colors.entries()].map(([hex, n]) => ({ hex, n })), roadDetail: rd, urbanDetail: ud });
})()`;
...
const page = b.page;
const out = { url: URL_, phase: PHASE, generatedAt: new Date().toISOString(), mission: '35X', spots: [], jsErrors: [] };
...
try {
  ...
  for (const sp of SPOTS) {
    ...
    console.log(`[35X-qa:${PHASE}] ${sp.id.padEnd(18)} calls=${String(perf.drawCalls).padStart(4)}`
