...
# 一括更新（既存の個別コマンドを順番に呼ぶだけの入口。新規取得処理は追加していない）
npm run data:update:all       # demographics → facilities を順次実行し、最後に4JSONの件数検証

# ローカルプレビュー（public/ をHTTP配信。データ更新はしない）
npm run preview               # http://localhost:8000/osaka_3d_buildings.html を配信
```

### 正規の運用手順

通常の確認（データはそのまま、表示だけ確認する）:
...
```bash
npm run data:update:all   # 先にデータを再生成
npm run preview           # その後プレビュー
```

- **HTMLを `file://` で直接開かないでください。** 統計は `map-data/…/*.json` をfetchで読むため、
  `file://` ではブラウザのセキュリティ制限で取得に失敗します。必ず `npm run preview`（HTTP配信）で開きます。
- **`public/map-data` は自動取得パイプラインの生成物です。** `npm run data:update:all` 等で再生成されます。
- **JSONを手作業で編集しないでください。** 変更は各データセット定義・変換スクリプト経由で行います。
- **プレビュー起動とデータ更新は別コマンドです。** `preview` は配信のみで、外部データの再取得や生成は一切しません。
- **データ更新に失敗しても既存の正常JSONは保護されます。** 書込は「一時ファイルへ出力→JSON妥当性を確認→
  同一ディレクトリ内でrenameして原子的に置換」する方式（`tools/lib/area.js` の `writeJsonSafely`）で、
  途中失敗時は一時ファイルを削除し、既存の正式ファイルには触れません。`data:update:all` は途中失敗で
  非ゼロ終了し、後続を実行しません（0件生成も失敗として扱います）。
